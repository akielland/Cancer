...
gene_freq_elastic <- function(matrix, name, n_models=1000){
  ...
  h <- ggplot(frequency[frequency$Frequency >= perc_best50,], aes(x = Frequency, y = reorder(Feature, Frequency))) +
...
gene_freq_elastic(e_c_obj_771_prolif$co_matrix, "elastic_90perc_r_p")
gene_freq_elastic(e_c_obj_771_RORprolif$co_matrix, "elastic_90perc_r_ROR")
