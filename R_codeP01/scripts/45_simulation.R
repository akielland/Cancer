...
syntetic_01 <- function(n, p_gr, p_non_gr, i){
  ...
  # Create matrices w group features
  ...
  # use model to create data
  y <- X %*% true_betas + interaction_effect + rnorm(n)
  colnames(y) <- "y"
...
char_list <- list(gr1 = char_group1,
                  gr2 = char_group2,
                  gr3 = char_group3,
                  gr4 = char_group4)


sim.data <- syntetic_01(n, p_gr, p_non_gr, i=1)
head(sim.data)
typeof(sim.data); class(sim.data)

# Run simulation
run_sim <- function(n_simulations){
  coef_matrix <- matrix(NA, nrow = n_simulations, ncol = 6)
  row_index <- 1
  for (i in c(1:n_simulations)){
    # i = i+12345
    set.seed(i) 
    data <- syntetic_01(n, p_gr, p_non_gr, i)
    out <- synergistic(data, char_list, alpha = 0.5, lambda_seq = NULL, nfolds = 5, tol = 1e-6, max_iters = 100)
...
n_simulations <- 100
t1 <- run_sim(n_simulations)
n_simulations <- 50
t2 <- run_sim(n_simulations)
...
mean_non_na <- apply(t2, 2, function(x) mean(x, na.rm = TRUE))
print("Mean of non-NA values:")
print(mean_non_na)

# Calculate the proportion of NA values in each column
mean_na <- apply(t2, 2, function(x) mean(is.na(x)))
mean_na
