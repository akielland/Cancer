...
########################################################
## ALL DATA from the chort (a test set for us) ##
########################################################
...
# extracting genes
ext.X <- df04 |> dplyr::select(42:812)
...
ext.PFS_months <- df04 |> dplyr::select(PFS_months)
ext.OS_months <- df04 |> dplyr::select(OS_months)



#######################################################
## 771 GENES + ROR categorical output from the trail ##
#######################################################

# select all gene names from original data table structure
genes <- colnames(df01 |> dplyr::select(41:811))
length(genes)
...
df_genes_ROR_classification <- function(df, predictors, output){
  ...
  df_genes <- df |> dplyr::select(all_of(features)) |>
    filter(timepoint=="SCR") |> 
    filter(TrialArmNeo=="Letro+Ribo")
  
  df_SUR <- df |> dplyr::select(c(UniqueID, timepoint, TrialArmNeo, output, ROR_P_GroupSubtypeProliferation)) |>
