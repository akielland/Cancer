...
elastic_net_interaction <- function(x_df, y, groups, alpha = 0.5, lambda_seq = NULL, nfolds = 5, tol = 1e-4, max_iters = 1000) {
  ...
  return(list(beta_main = named_beta_main, beta_interaction = named_beta_interaction, iterations = iter, best_lambda = best_lambda))
...
interaction_effect <- (X_df$X1 * X_df$X6) * 0.5
X_mat <- as.matrix(X_df)
true_betas_mat <- matrix(true_betas, ncol = 1)
y <- X_mat %*% true_betas_mat + interaction_effect + rnorm(n)
...
result <- elastic_net_interaction(X_df, y, char_list, alpha = 0.5, lambda_seq = NULL, nfolds = 5, tol = 1e-5, max_iters = 100)
# Print the results
print(result)
