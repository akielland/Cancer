...
lasso_rep_cv <- function(df_data, folds=5, repeats=1, method="pearson") {
  ...
  for (i in 1:repeats) {
    ...
    for (j in 1:folds) {
      ...
      X_prolif <- as.matrix(train_data[c(char_prolif)])
      X_immune_inf <- as.matrix(train_data[c(char_immune_inf)])
      X_ER_singaling <- as.matrix(train_data[c(char_ER_signaling)])
    
      fit_prolif <- cv.glmnet(X_prolif, train_data$Y, nfolds=5)
      fit_immune_inf <- cv.glmnet(X_immune_inf, train_data$Y, nfolds=5)
      fit_ER_singaling <- cv.glmnet(X_ER_singaling, train_data$Y, nfolds=5)
      
      pred_prolif = predict(fit_prolif, newx = as.matrix(train_data[c(char_prolif)]), type = "response", s = "lambda.min")
      pred_immune = predict(fit_immune_inf, newx = as.matrix(train_data[c(char_immune_inf)]), type = "response", s = "lambda.min")
      pred_ER_signal = predict(fit_ER_singaling, newx = as.matrix(train_data[c(char_ER_signaling)]), type = "response", s = "lambda.min")
